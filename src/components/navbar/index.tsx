...
const Navbar = () => {
  ...
  return (
    <div className='grid grid-cols-2 lg:grid-cols-3 p-6 fixed top-0
    left-0 right-0 z-50'>
        <div className='flex items-center gap-4'>
            <Link
            href={`/dashboard`}
            className='w-8 h-8 rounded-full border-3 border-white bg-black
            ...
            {!hasCanvas || 
            (!hasStyleGuide && (
                <div className='lg:inline-block hidden rounded-full text-primary/60 border border-white/12
                backdrop-blur-xl bg-white/8 px-4 py-2 text-sm saturate-150'>
        ...
        <div className='lg:flex hidden items-center justify-center gap-2'>
            <div className='flex items-center gap-2 backdrop-blur-xl bg-white/8 border border-white/12
            rounded-full p-2 saturate-150'>
                {tabs.map((t) => (
                    <Link
                    key={t.href}
                    href={t.href}
                    className={[
                        'group inline-flex items-center gap-2 rounded-full px-4 py-4 text-sm transition', 
        ...
        <div className='flex items-center gap-4 justify-end'>
            ...
            backdrop-blur-xl bg-white/8 border border-white/12 saturate-150
            hover:bg-white/12'
            >
